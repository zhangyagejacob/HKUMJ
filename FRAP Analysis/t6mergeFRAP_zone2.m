...
for i = 1 : length( dirs_2 )
    ...
    for j = 1 : length( dirs_3 )
        ...
        for k = 1:length(dirs_4)
            ...
            dir_file_CH1_zone2 = fullfile(dir_name_4, 'FRAP_CH1_zone2.txt');
            dir_file_CH2_zone2 = fullfile(dir_name_4, 'FRAP_CH2_zone2.txt');
            fid_CH1_zone2 = zeros(100,1);
            fid_CH2_zone2 = zeros(100,1);
            for l = 1:length(dirs_5)
                ...
                for m = 1:length(dirs_6)
                    ...
                    if( exist( fullfile(dir_name_5,'./zone2/FRAP-BKC-CH1-zone2_alignment.txt'),'file'))
                        A_CH1 = zeros(100,1);
                        B_CH1 = importdata(fullfile(dir_name_5, './zone2/FRAP-BKC-CH1-zone2_alignment.txt'));
                        A_CH1(1:numel(B_CH1),1) = B_CH1;
                        fid_CH1_zone2 = [fid_CH1_zone2, A_CH1]
                    end
                    if( exist( fullfile(dir_name_5,'./zone2/FRAP-BKC-CH2-zone2_alignment.txt'),'file'))
                        A_CH2 = zeros(100,1);
                        B_CH2 = importdata(fullfile(dir_name_5, './zone2/FRAP-BKC-CH2-zone2_alignment.txt'));
                        A_CH2(1:numel(B_CH2),1) = B_CH2;
                        fid_CH2_zone2 = [fid_CH2_zone2, A_CH2];
                    end
                end
            end
            fid_CH1_zone2 = fid_CH1_zone2(:,2:end);
            fid_CH2_zone2 = fid_CH2_zone2(:,2:end);
            dlmwrite(dir_file_CH1_zone2, fid_CH1_zone2, 'delimiter', ' ', 'precision', 15);
            dlmwrite(dir_file_CH2_zone2, fid_CH2_zone2, 'delimiter', ' ', 'precision', 15);
        end
    end
end