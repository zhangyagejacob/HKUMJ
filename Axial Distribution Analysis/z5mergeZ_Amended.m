...
for i = 1 : length( dirs_2 )
    ...
    for j = 1 : length( dirs_3 )
        ...
        for k = 1:length(dirs_4)
            ...
            dir_file_1 = fullfile(dir_name_4, 'Z-CH1-Amended.txt');
            dir_file_2 = fullfile(dir_name_4, 'Z-CH2-Amended.txt');
            dir_file_3 = fullfile(dir_name_4, 'Z-CH3-Amended.txt');
            fid_CH1 = zeros(22,1);
            ...
            for l = 1:length(dirs_5)
                ...
                for m = 1:length(dirs_6)
                    ...
                    %% CH1.txt
                    if( exist( fullfile(dir_name_6, 'BK-CH1-Amended.txt'),'file'))                  
                        A = zeros(22,1);
                        % % open file BK-CH1.txt, read only
                        B = importdata(fullfile(dir_name_6, 'BK-CH1-Amended.txt'));   
                        A(1:size(B,1),1) = B;
                    ...
                    %% CH2.txt
                    if( exist( fullfile(dir_name_6, 'BK-CH2-Amended.txt'),'file'))                    
                        A = zeros(22,1);
                        % % open file 00BK-CH1.txt, read only
                        B = importdata(fullfile(dir_name_6, 'BK-CH2-Amended.txt'));   
                        A(1:size(B,1),1) = B;
                        % combine B to the new column of fid, and called this new matrix C
                        fid_CH2 = [fid_CH2, A];
                    end
                    ...
                    %% CH3.txt
                    if( exist( fullfile(dir_name_6, 'BK-CH3-Amended.txt'),'file'))                    
                        A = zeros(22,1);
                        % % open file 00BK-CH1.txt, read only
                        B = importdata(fullfile(dir_name_6, 'BK-CH3-Amended.txt'));   
                        A(1:size(B,1),1) = B;
                        % combine B to the new column of fid, and called this new matrix C
                        fid_CH3 = [fid_CH3, A];
                    end
