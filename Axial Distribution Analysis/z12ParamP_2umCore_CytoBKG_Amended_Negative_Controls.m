...
for i = 1 : length( dirs_2 )
    ...
    for j = 1 : length( dirs_3 )
        ...
        for k = 1:length(dirs_4)
            ...
            dir_file_CH1_Ppara = fullfile(dir_name_4, 'P-Parameter-CH1-2016-01-09-Avg2um-CytBKG.txt');
            dir_file_CH2_Ppara = fullfile(dir_name_4, 'P-Parameter-CH2-2016-01-09-Avg2um-CytBKG.txt');
%             dir_file_CH3_Ppara = fullfile(dir_name_4, 'P-Parameter-CH3-2016-01-09-Avg2um-CytBKG.txt');
...
%                 CytosolRAW_CH3 = CytosolRAW.data(3,3);
                ...
                for m = 1:length(dirs_6)    % for PDXs
                    ...
                    if( exist( fullfile(dir_name_6, 'CH1_alignment.txt'),'file'))
                        ...
                        I_11frames = ParaRaw.data(1:11,3) - CytosolRAW_CH1;
                        for n = 1:length(I_11frames)
                            if(I_11frames(n) < 0)
                                I_11frames(n) = 0;
                            end
                        end
                        P_Avg = mean(I_11frames);
                        P_Avg_RAW = mean(ParaRaw.data(1:11,3));
                        P = P_Avg / (P_Avg_RAW - nonCellBKG_CH1);
...
%                         CH1_Parameters(length(CH1_Parameters)).Min_Frame = min_frame;
                    ...
                    if( exist( fullfile(dir_name_6, 'CH2_alignment.txt'),'file'))
                        ...
                        I_11frames = ParaRaw.data(1:11,3) - CytosolRAW_CH2;
                        for n = 1:length(I_11frames)
                            if(I_11frames(n) < 0)
                                I_11frames(n) = 0;
                            end
                        ...
                        P_Avg = mean(I_11frames);
                        P_Avg_RAW = mean(ParaRaw.data(1:11,3));
                        P = P_Avg / (P_Avg_RAW - nonCellBKG_CH2);
...
%                         I_11frames = ParaRaw.data(1:11,3) - CytosolRAW_CH3;
%                         for n = 1:length(I_11frames)
%                             if(I_11frames(n) < 0)
%                                 I_11frames(n) = 0;
%                             end
...
%                         P_Avg = mean(I_11frames);
%                         P_Avg_RAW = mean(ParaRaw.data(1:11,3));
%                         P = P_Avg / (P_Avg_RAW - nonCellBKG_CH3);
