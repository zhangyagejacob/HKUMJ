...
for i = 1 : length( dirs_2 )
    ...
    for j = 1 : length( dirs_3 )
        ...
        for k = 1:length(dirs_4)
            ...
            dir_file_CH1_Ppara = fullfile(dir_name_4, 'P-Parameter-CH1-1um-Amended-2015-11-19-2um.txt');
            dir_file_CH2_Ppara = fullfile(dir_name_4, 'P-Parameter-CH2-1um-Amended-2015-11-19-2um.txt');
            %dir_file_CH3_Ppara = fullfile(dir_name_4, 'P-Parameter-CH3-1um-Amended-2015-11-19-2um.txt');
            ...
            for l = 1:length(dirs_5)
                ...
                for m = 1:length(dirs_6)    % for PDXs
                    ...
                    if( exist( fullfile(dir_name_6, 'CH1_alignment.txt'),'file'))
                        ...
                        if(CytosolBKGsub(1) < ParaRaw.data(11,3))
                            P_xum = ParaRaw.data(1:6,3);
                            P_Bar = mean(P_xum);
                            P = (P_Bar - CytosolRAW_CH1) / (P_Bar - nonCellBKG_CH1);
                            %RESULT = [P];
                            %CH1_Parameters = [CH1_Parameters;RESULT];
                    ...
                    if( exist( fullfile(dir_name_6, 'CH2_alignment.txt'),'file'))
                        ...
                        if(CytosolBKGsub(2) < ParaRaw.data(11,3))
                            P_xum = ParaRaw.data(1:6,3);
                            P_Bar = mean(P_xum);
                            P = (P_Bar - CytosolRAW_CH2) / (P_Bar - nonCellBKG_CH2);
                            %RESULT = [P];
                            %CH2_Parameters = [CH2_Parameters;RESULT];
...
%                         if(CytosolBKGsub(3) < ParaRaw.data(11,3))
%                             P_xum = ParaRaw.data(1:6,3);
%                             P_Bar = mean(P_xum);
%                             P = (P_Bar - CytosolRAW_CH3) / (P_Bar - nonCellBKG_CH3);
%                             %RESULT = [P, dir_name_6];
%                             %CH3_Parameters = [CH3_Parameters;RESULT];
...
%                     end
            ...
            %dlmwrite(dir_file_CH1_Ppara, CH1_Parameters, 'delimiter', ' ', 'precision', 15);
            %dlmwrite(dir_file_CH2_Ppara, CH2_Parameters, 'delimiter', ' ', 'precision', 15);
            %dlmwrite(dir_file_CH3_Ppara, CH3_Parameters, 'delimiter', ' ', 'precision', 15);
