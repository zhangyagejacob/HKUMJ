        ...
        for k = 1:length(dirs_4)
            ...
            for l = 1:length(dirs_5)
                ...
                for m = 1:length(dirs_6)
                    ...
                    Z4zone_CH3 = dlmread(fullfile(dir_name_6,'CH3-mscan.txt'),'\t', 1, 0);
                    dir_CH1 = fullfile(dir_name_6, 'CH1_Zone_Analysis.txt');
                    dir_CH2 = fullfile(dir_name_6, 'CH2_Zone_Analysis.txt');
                    dir_CH3 = fullfile(dir_name_6, 'CH3_Zone_Analysis.txt');
                    
                    %% Fatch Zone 2 from F-actin Channel 3 by determining Xf and Xr
                    [Ip3,Xp3] = max(Z4zone_CH3(:,2));
                    Ic3 = Icyb(3) + 3*sdCyb(3);         % use statistical methods to find Podosome region
                    Xf = min(find(Z4zone_CH3(:,2)>Ic3 & Z4zone_CH3(:,1)<Xp3));
                    Xr = max(find(Z4zone_CH3(:,2)>Ic3 & Z4zone_CH3(:,1)>Xp3));
                    
                    %% Get [Ip1, Xp1] & [Ip2, Xp2]
                    % find local maximum alpha
                    if (sum(Z4zone_CH1(Xf:Xp3,2)>Z4zone_CH1(Xp3,2))>0)
                        [Ip1,Xp1] = max(Z4zone_CH1(Xf:Xp3,2));
                        Xp1 = Xp1+Xf-1;
                        if sum(Z4zone_CH1(Xp1-2:Xp1+2))/5 <= Z4zone_CH1(Xp3,2)
                    ...
                    if (sum(Z4zone_CH2(Xf:Xp3,2)>Z4zone_CH2(Xp3,2))>0)
                        [Ip2,Xp2] = max(Z4zone_CH2(Xf:Xp3,2));
                        Xp2 = Xp2+Xf-1;
                        if sum(Z4zone_CH2(Xp2-2:Xp2+2,2))/5 <= Z4zone_CH2(Xp3,2)
                    ...
                    PIR1 = (Ip1-Icyb(1)) / (Z4zone_CH1(Xp3,2)-Icyb(1));
                    PIR2 = (Ip2-Icyb(2)) / (Z4zone_CH2(Xp3,2)-Icyb(2));
                    PIR3 = 1;
